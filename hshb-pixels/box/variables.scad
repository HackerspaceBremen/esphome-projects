...
display_pcb_hole_distance_x_abs =
    (display_pcb_width - display_pcb_hole_distance_x) / 2;
display_pcb_hole_distance_z_abs =
    (display_pcb_height - display_pcb_hole_distance_z) / 2;
...
display()
{
    ...
    translate([
        display_pcb_hole_distance_x_abs,
        display_pcb_hole_length / 2,
        display_pcb_hole_distance_z_abs
    ]) rotate([ 90, 0, 0 ])
        cylinder(h = display_pcb_hole_length, d = display_pcb_hole_diameter);

    translate([
        display_pcb_hole_distance_x_abs + display_pcb_hole_distance_x,
        display_pcb_hole_length / 2,
        display_pcb_hole_distance_z_abs
    ]) rotate([ 90, 0, 0 ])
        cylinder(h = display_pcb_hole_length, d = display_pcb_hole_diameter);
    ...
    translate([
        (display_pcb_width - display_width) / 2,
        display_pcb_depth - 0.001,
        (display_pcb_height - display_height) / 2
    ]) cube([ display_width, display_depth, display_height ]);

    translate([
        (display_pcb_width - display_pcb_contacts_x -
         display_pcb_contacts_width),
        -display_pcb_contacts_depth,
        display_pcb_height - display_pcb_contacts_z -
        display_pcb_contacts_height
    ])
        cube([
            display_pcb_contacts_width,
            display_pcb_contacts_front_depth + display_pcb_depth +
                display_pcb_contacts_depth,
            display_pcb_contacts_height
        ]);
}
...
lid()
{
    difference()
    {
        ...
        translate([ box_width - 10, 20, -lid_thickness ])
            cylinder(h = 3 * lid_thickness, d = 8);
        translate([ box_width - 10, 11, -lid_thickness ])
            cylinder(h = 3 * lid_thickness, d = 5);
    }
}
